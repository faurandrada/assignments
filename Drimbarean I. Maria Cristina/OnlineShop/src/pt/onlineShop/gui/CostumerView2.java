...
public class CostumerView2 extends JFrame{
	private static final long serialVersionUID = 3657411020433775472L;
	private OPDept department;
	JTextField nameSearch,nameOrder,amount;
	JButton search,order,show,exit;
	public CostumerView2(OPDept department){
	...
	private class Handler implements ActionListener{
		...
		public void actionPerformed(ActionEvent event) {
			if (event.getSource()==search){
				...
				department.getCostumer2().searchProduct(department.getWarehouse(), productName);
			}else if (event.getSource()==order){
				String productName=nameOrder.getText();
				int value=Integer.parseInt(amount.getText());
				department.getCostumer2().placeOrder(department.getWarehouse(), productName, value);
			}
			else if (event.getSource()==show){
				new OrderHistory(department.getCostumer2().getOrders());
